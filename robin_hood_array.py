...
class RobinHoodArrayAnimationManual(Scene):
    ...
    def construct(self):
        ...
        title2 = Text("Insertion Process", font_size=36, color=WHITE).next_to(title1, DOWN, buff=0.5)
        ...
        # ─── 1) SMALL HEADER ─────────────────────────────────────────────────
        header_title = Text("Robin Hood Addressing", font_size=36, color=BLUE).to_edge(UP).shift(DOWN * 1.0)
        header_text = Text(
            ...
            line_spacing=0.4,
            color=WHITE
        ).next_to(header_title, DOWN, aligned_edge=LEFT, buff=0.2)
        ...
        # Helper to show notification (1.5 seconds)
